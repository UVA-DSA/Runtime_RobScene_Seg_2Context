    ...
    def DrawSingleImageContextS(self,pred, gt,needleShape,needleShape_gt,GrasperJawPoints,imageFName,outputFName,CtxI,ctxPredLine,frameNumber,L_Gripping,R_Gripping,LG_inter_T, RG_inter_T,messages,GT=False):
        ...
        for thread in T_dl.geoms:
            if thread.exterior.xy:
                ...
                x = [xx*scale for xx in x[:-1]]
                y = [yy*scale for yy in y[:-1]]
                plt.plot(x,y,color="blue",linewidth=linewidth)
        

        '''
        unaryThread =make_valid( unary_union(T_dl) )

        try:
            cords = unaryThread.coords if len(T_dl) > 1 else [0,0]
            #plt.plot(cords)
            plt.plot(cords,color="blue",linewidth=linewidth)
        except Exception as e:
            print("Thread errpr:",e)
        '''
        ...
        colors = ["black"]
        for i in range(1,len(ctxGT)):
            if ctxGT[i] != ctxPred[i]:
                colors.append("red")
            else:
                colors.append("black")
        ...
        #mpt.rcParams.update(mpt.rcParamsDefault)
        #rc('text', usetex = True)
        #rc("text.latex", preamble=r"\usepackage{color}")
        contextMessage = ['Ground Truth:',' '.join(ctxGT),'','Predicted Context:']
        offset = 1
        fs = 7
        ctxOffset = 360
        caxis = plt.gca()
        ...
        x = 10
        y = ctxOffset + 22 * offset; 
        for index in range(len(ctxPred)):
            if(colors[index] == "black"):
                plt.text(x,y,ctxPred[index],fontsize=fs,color=colors[index])
            else:
                plt.text(x,y,ctxPred[index],fontsize=fs,color=colors[index],weight="bold")

            if index > 0:
                x+= 13*len(ctxPred[index])
            else:
                x+= 8*len(ctxPred[index])
