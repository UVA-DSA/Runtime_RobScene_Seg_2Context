...
class StaticTransformDataset(Dataset):
    ...
    def __init__(self, root, method=0):
        ...
        self.method = method
            # Get images
        self.im_list = []
        classes = os.listdir(path.join(self.root,'Images'))
        for c in classes:
            imgs = os.listdir(path.join(root,'Images', c))
            joint_list = [path.join(root, 'Images', c, im) for im in imgs]
        ...
        self.all_gt_dual_transform = transforms.Compose([transforms.RandomAffine(degrees=0, scale=(0.8, 1.5), fill=0),
            transforms.RandomHorizontalFlip(),
        ])
    ...
    def __getitem__(self, idx):
        ...
        gt = Image.open(self.im_list[idx].replace('Images','Annotations')).convert('L')
        ...
        for _ in range(3):
            ...
            this_gt = self.final_gt_transform(this_gt)  #Converts a PIL Image or numpy.ndarray (H x W x C) in the range [0, 255] to a 
        ...
        cls_gt[masks[:,0] > 0.05] = 1 # changed threshold to accomodate lower label value
