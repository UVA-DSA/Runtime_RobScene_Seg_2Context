...
class GenericTestDataset(Dataset):
    def __init__(self, imagedir,maskdir,task, window, res=480):
        ...
        for vid in vid_list:
            ...
            first_mask = os.listdir(path.join(self.mask_dir, vid))
    ...
    def __getitem__(self, idx):
        ...
        for subframe in range(0, lf , window):
            ...
            for idx in range(subframe,end_frame): 
                f = frames[idx]
                mask_file = path.join(vid_gt_path, f.replace('.png','_pred.png')) # deeplab
